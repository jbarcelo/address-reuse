...
For a regular transaction to be valid, it is required that the total value of the outputs does not exceed the total amount of the inputs.
...
Other important element of Bitcoin are public/private asymmetric cryptographical keys.
...
A Bitcoin address can be used used multiple times to receive transaction outputs.
...
The address space is 160 bits which means that the chances of address collision can be neglected for practical purposes.
...
If the value of the inputs is larger than the output, it is likely that the sending part does not want to loose the difference.
The difference between the total value of the inputs and the desired value of the outputs is called the change.
...
It may be tempting to use  Bitcoin addresses like regular bank accounts numbers.
...
Address re-use increases the chances that an address is linked to its owner and also increases
The original Bitcoin paper \cite{nakamoto2008bpp} discourages address reuse.

\subsection{Address Reuse in the Blockchain}

Despite the fact that address re-use was discouraged since the inception of Bitcoin, we can find evidence of address reuse in the Blockchain.
...
If the donor uses the default reused address, it is likely that some privacy information is leaked. 
...
A second example is when the owner uses that address to receive payments.
If the payer knows both the payee and the address, the address is no longer pseudonymous and can be associated to a real identity.
Similarly, if the address owner uses it for a non-anonymous payment, the identity of the owner also becomes linked to the address.
...
This is not a problem if the donors are aware of this situation and know that Bitcoin, on its own, does not protect the privacy of its users.
...
If the inputs are of similar size, a payee cannot determine which inputs belong to which payer.
...
We present data regarding address re-use in the blockchain as well as possible CoinJoin transactions.
