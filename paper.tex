...
A Bitcoin transaction transfers funds from one or more input transactions to one or more output addresses.
In the most usual case, an address is linked to a private key.
...
The addresses that are linked to a single private key are alphanumeric (Bese58) chains starting by 1.
Addresses are around 30 characters long.
For convenience, in our examples we will use shorter addresses such as \texttt{1Ann}.
Imagine that Ann transfers one bitcoin from her address \texttt{1Ann} to Bob's address \texttt{1Bob}.
Bob wants to transfer that bitcoin to Chris' address \texttt{1Chris}.
As Bob has the private key associated to the address \texttt{1Bob}, he can create a transaction that has as an input Ann's transaction and outputs the funds to \texttt{1Chris}.
